...
def jita():
    ...
    cur.execute("SELECT name, price, profit, ROUND(profitmargin), mytime, cost, p_level, buy_price, buy_cost FROM name, temp_jita WHERE itemid = id;")
...
def amarr():
    ...
    con = connection.establish_connection()
    cur = con.cursor()
    cur.execute("SELECT name, price, profit, ROUND(profitmargin), mytime, cost, p_level, buy_price, buy_cost FROM name, temp_amarr WHERE itemid = id;")
    entries = cur.fetchall()
    cur.close()
    con.close()
...
def rens():
   # return render_template('maintenance.html')
    con = connection.establish_connection()
    cur = con.cursor()
    cur.execute("SELECT name, price, profit, ROUND(profitmargin), mytime, cost, p_level, buy_price, buy_cost FROM name, temp_rens WHERE itemid = id;")
    entries = cur.fetchall()
    cur.close()
    con.close()
...
def dodixie():
   # return render_template('maintenance.html')
    con = connection.establish_connection()
    cur = con.cursor()
    cur.execute("SELECT name, price, profit, ROUND(profitmargin), mytime, cost, p_level, buy_price, buy_cost FROM name, temp_dodixie WHERE itemid = id;")
    entries = cur.fetchall()
    cur.close()
    con.close()
